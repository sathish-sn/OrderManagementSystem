...
public class Methods {
	...
	static public ArrayList<Order> getOrderList() {
		...
		try {
			...
			while (readfile.hasNextLine()) {
				...
				Order object = new Order(orderId, orderDescription, deliveryAddress, orderDate, amount, deliveryDatetime,
						status);
	...
	outer :while(flag==1) {
		...
		Order obj = new Order(orderId, orderDescription, deliveryAddress, orderDate, amount, deliveryDateTime, status);
		list.add(obj);
   ...
   return 0;
	...
	//	DateFormat dtf = new SimpleDateFormat("yyyy/MM/dd_HH:mm:ss");
		...
		for (Order getValue : viewList) {
			...
			if (getValue.getOrderID().equals(markAsDel) && getValue.getStatus().equals("Delivered")) {
